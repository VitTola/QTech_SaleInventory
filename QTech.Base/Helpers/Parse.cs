    ...
    public class Parse
    {
        ...
        public static Bitmap ToImage(byte[] bytes)
        {
            ...
            if (bytes != null)
            {
                if (bytes.Length > 0)
                {
                    //Open Stream for image and write byte into it
                    System.IO.MemoryStream objMemoryStream = new MemoryStream(bytes, true);
                    objMemoryStream.Write(bytes, 0, bytes.Length);

                    //Create bitmap from memory stream
                    bitmap = new Bitmap(objMemoryStream);
                }
            }
            return bitmap;
        }
        ...
        public static Binary ToBinary(Image image)
        {
            System.IO.MemoryStream ms = new System.IO.MemoryStream();
            try
            {
                image.Save(ms, System.Drawing.Imaging.ImageFormat.Jpeg);
                Binary binReturn = new Binary(ms.ToArray());
                return binReturn;
            }
            catch (NullReferenceException)
            {
        ...
        public static Bitmap ToImage(Binary binary)
        {
            object x = null;
            byte[] y = (byte[])x;
            ...
            if (binary != null)
            {
                ...
                {
                    //Open Stream for image and write byte into it
                    System.IO.MemoryStream objMemoryStream = new MemoryStream(binary.ToArray(), true);
                    objMemoryStream.Write(binary.ToArray(), 0, binary.Length);
