...
namespace QTech.Forms
{
    public partial class frmSale : ExDialog, IDialog
    {
        ...
        {
            if (dgv.RowCount == 0 && dgv.CurrentCell.ColumnIndex == dgv.Columns.IndexOf(colEmployeeId))
            {
                return;
            }
            dgv.EndEdit();
        ...
        private async void CheckValidProductByPO()
        {
            ...
            if (purchaseOrderNo != null)
            {
                ...
                {
                    var inputQty = Parse.ToDecimal(dgv.CurrentRow.Cells[colQauntity.Name].Value?.ToString() ?? "0");

                    var _productId = Parse.ToInt(dgv.CurrentRow.Cells[colProductId.Name].Value?.ToString() ?? "0");
                    if (_productId != 0)
                    {
